...
- 📊 **全デバイスのバッテリー状況を毎週レポート**
- 🎨 色分け表示（🟢 正常 / 🔴 要交換）
- 📱 Slack への自動通知
- ⏰ GitHub Actions による定期実行（**毎週月曜 9:00 JST**）
...
```
🔋 SwitchBot バッテリーレポート (2025-11-19)
...
```yaml
schedule:
  # 毎週水曜 12:00 JST (3:00 UTC) に実行する場合
  - cron: '0 3 * * 3'
...
設定したスケジュール（デフォルト: **毎週月曜 9:00 JST**）で自動的に実行されます。
